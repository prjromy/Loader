...
namespace Loader.Models
{
    ...
    public class RegisterViewModel
    {
        ...
        [EmailAddress]
        [Display(Name = "Email")]
        [Remote("CheckEmailAvailable","Users",AdditionalFields ="Id",ErrorMessage ="Duplicate Email entry!!!")]
        [RegularExpression("^[a-z0-9_\\+-]+(\\.[a-z0-9_\\+-]+)*@[a-z0-9-]+(\\.[a-z0-9-]+)*\\.([a-z]{2,4})$", ErrorMessage = "Not a valid email")]
        public string Email { get; set; }

        public Nullable<int> EmployeeId { get; set; }
        [Required]
        [Remote("CheckUsernameAvailable","Users",AdditionalFields ="Id",ErrorMessage ="Duplicate Username entry!!!")]
      //  [Required(ErrorMessage = "UserName Is Required"), MaxLength(100), Column(TypeName = "nvarchar")]
        ...
        [DataType(DataType.Password)]
        [Display(Name = "Password")]
        [RegularExpression(pattern: "^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$", ErrorMessage ="Password must required at least one special character one lower case one uppercase and at least one number ")]
        public string Password { get; set; }
